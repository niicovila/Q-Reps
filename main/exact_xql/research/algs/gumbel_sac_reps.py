...
def qreps_loss(pred, label, beta, clip, gamma, value):
    ...
    return torch.mean(beta * torch.log(loss)) + (1-gamma)*value.mean()
...
class GumbelSACREPS(Algorithm):
    ...
    def _train_step(self, batch):
        ...
        if self.steps % self.critic_freq == 0:
            ...
            q_loss = sum([qreps_loss(qs[i], target_q, self.beta, batch['discount'][0], value) for i in range(qs.shape[0])])
            print(q_loss)
