...
def tune_elbe(config):
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        if args.save_learning_curve and len(reward_iteration) > 0: rewards_df = rewards_df._append({"Step": global_step, "Reward": np.mean(reward_iteration)}, ignore_index=True)
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.total_iterations, args.minibatch_size):
                    ...
                    critic_loss = eta * torch.log(torch.mean(torch.exp(delta / eta), 0)) + torch.mean((1 - args.gamma) * val, 0)
