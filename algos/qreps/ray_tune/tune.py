...
from train_func_elbe import tune_elbe, tune_elbe_decoupled, tune_elbe_target
# from train_func_saddle_atari import tune_saddle_atari, tune_saddle_atari_decoupled
# from train_func_elbe_atari import tune_elbe_atari, tune_elbe_atari_decoupled
...
# print(ray.nodes())
scheduler = AsyncHyperBandScheduler(grace_period=20, time_attr="training_iteration", max_t=100)
num_samples = 512
seeds = 1
...
if optuna:
    ...
    tuner = tune.Tuner(  
        tune.with_resources(tune_elbe_target, resources={"cpu": 1}),
        tune_config=tune.TuneConfig(
            metric="reward",
            mode="max",
            search_alg=search_alg,
    ...
    print("Best config is:", result_grid.get_best_result().config)
    results_df = result_grid.get_dataframe()
    results_df.to_csv(f"tune_results_{run}_{num_samples}_target.csv")
